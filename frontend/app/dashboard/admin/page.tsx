...
const statusColors: Record<string, { bg: string; color: string }> = {
    ...
    deal_closed: { bg: 'rgba(168,85,247,0.12)', color: '#a855f7' },
    cancelled:   { bg: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)' },
    expired:     { bg: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)' },
};
const roleColors: Record<string, { bg: string; color: string }> = {
    ...
    brand:      { bg: 'rgba(168,85,247,0.12)',  color: '#a855f7' },
...
const Badge = ({ label, map }: { label: string; map: Record<string, { bg: string; color: string }> }) => {
    const s = map[label] || { bg: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)' };
    return (
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {activeTab === 'users' && (
                            <motion.div key="users" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                                ...
                                <div style={{ display: 'flex', gap: 10, marginBottom: 18, flexWrap: 'wrap', alignItems: 'center' }}>
                                    <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
                                        ...
                                        <input value={userSearch} onChange={e => setUserSearch(e.target.value)} placeholder="Search by name or email..."
                                            style={{ width: '100%', padding: '10px 12px 10px 34px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', color: '#ffffff', fontSize: 13, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
                                    </div>
                                    <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
                                        style={{ padding: '10px 14px', borderRadius: 12, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', color: '#ffffff', fontSize: 12, fontFamily: 'inherit', cursor: 'pointer', outline: 'none' }}>
                                ...
                                {/* Users table */}
                                <div style={{ background: '#0c0c0c', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 24, overflow: 'hidden', boxShadow: '0 8px 30px rgba(0,0,0,0.2)' }}>
                                    <div style={{ padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <h2 style={{ fontFamily: 'Space Grotesk', fontWeight: 700, fontSize: 14, color: '#ffffff' }}>All Users ({users.length})</h2>
                                    </div>
                                    ...
                                    {loading ? (
                                        ...
                                        <div style={{ padding: 60, textAlign: 'center', color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>No users found</div>
                                    ) : (
                                                ...
                                                <thead>
                                                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                                        {['User', 'Role', 'Status', 'Profile', 'Joined', 'Actions'].map(h => (
                                                            <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontSize: 10, fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>{h}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                            ...
                                                            <td style={{ padding: '14px 16px' }}>
                                                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                                    {u.instagramProfilePicture ? (
                                                                        ...
                                                                        <div style={{ width: 34, height: 34, borderRadius: '50%', background: 'linear-gradient(135deg,#9333ea,#c084fc)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, flexShrink: 0 }}>
                                                ...
                                                <tbody>
                                                    {filteredCampaigns.map((c) => (
                                                        <tr key={c._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', transition: 'background 150ms' }}
                                                            ...
                                                            {/* Brand */}
                                                            <td style={{ padding: '14px 16px' }}>
                                                                <p style={{ fontSize: 12, color: '#a855f7' }}>{c.brandName || '—'}</p>
                                                            </td>
                                                            ...
                                                            <td style={{ padding: '14px 16px' }}>
                                                                <div style={{ display: 'flex', gap: 6 }}>
                                                                    ...
                                                                    <button onClick={(e) => { e.stopPropagation(); handleCampaignStatus(c._id, 'deal_closed'); }} title="Close Deal" style={{ padding: '6px', borderRadius: 8, background: 'rgba(168,85,247,0.1)', border: '1px solid rgba(168,85,247,0.2)', color: '#c084fc', cursor: 'pointer' }}><CheckCircle size={13} /></button>
