                    ...
                    <AnimatePresence initial={false}>
                        {messages.map((msg, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                                ...
                                {msg.role === 'ai' && (
                                    <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: 'linear-gradient(135deg, #7B3FF2, #A855F7)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 0 15px rgba(123,63,242,0.3)' }}>
                                        <Bot size={16} color="#fff" />
                                    </div>
                                )}
                                <div style={{ padding: '14px 18px', borderRadius: '16px', background: msg.role === 'user' ? 'linear-gradient(135deg, rgba(255,255,255,0.1), rgba(255,255,255,0.05))' : 'rgba(123,63,242,0.1)', border: msg.role === 'user' ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(123,63,242,0.2)', color: msg.role === 'user' ? '#fff' : '#e9d5ff', fontSize: '14px', lineHeight: '1.6', borderBottomRightRadius: msg.role === 'user' ? '4px' : '16px', borderBottomLeftRadius: msg.role === 'ai' ? '4px' : '16px' }}>
                                    {msg.text}
                                </div>
                                {msg.role === 'user' && (
                        ...
                        {loading && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                                <div style={{ width: '32px', height: '32px', borderRadius: '10px', background: 'linear-gradient(135deg, #7B3FF2, #A855F7)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    <Bot size={16} color="#fff" />
                                </div>
                                <div style={{ padding: '12px 18px', borderRadius: '16px', background: 'rgba(123,63,242,0.05)', border: '1px solid rgba(123,63,242,0.1)' }}>
                ...
                <div style={{ background: 'rgba(0,0,0,0.2)', borderTop: '1px solid rgba(255,255,255,0.06)', padding: '16px 24px', display: 'flex', gap: '12px', alignItems: 'center' }}>
                    <div style={{ position: 'relative', flex: 1 }}>
                        <input
                            ...
                            onFocus={(e) => e.target.style.border = '1px solid rgba(123,63,242,0.5)'}
                            onBlur={(e) => e.target.style.border = '1px solid rgba(255,255,255,0.1)'}
                        />
                        <button
                            onClick={handleSend}
                            disabled={loading || !input.trim()}
                            style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', padding: '8px', borderRadius: '10px', background: loading || !input.trim() ? 'rgba(255,255,255,0.05)' : 'linear-gradient(135deg, #7B3FF2, #A855F7)', border: 'none', color: loading || !input.trim() ? 'rgba(255,255,255,0.2)' : '#fff', cursor: loading || !input.trim() ? 'not-allowed' : 'pointer', transition: 'all 0.2s' }}
                        >
                            <Send size={16} />
                        </button>
                    </div>
