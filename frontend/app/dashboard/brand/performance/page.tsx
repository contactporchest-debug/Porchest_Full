...
function StatCard({ label, value, hint, tone }: { label: string; value: string; hint?: string; tone: string }) {
    return (
        <div style={{ borderRadius: 18, border: `1px solid ${BORDER}`, background: 'rgba(255,255,255,0.42)', padding: 18 }}>
...
function SmallMetric({ label, value, tone }: { label: string; value: string; tone: string }) {
    return (
        <div style={{ borderRadius: 16, border: `1px solid ${BORDER}`, background: 'rgba(255,255,255,0.72)', padding: 16 }}>
...
export default function BrandPerformancePage() {
    ...
    const loadPerformance = async (silent = false) => {
        ...
        try {
            const res = await brandAPI.getCampaignPerformance();
                            ...
                            {campaigns.length === 0 ? (
                                ...
                                <div style={{ overflowX: 'auto' }}>
                                    <table style={{ width: '100%', minWidth: 980, borderCollapse: 'separate', borderSpacing: 0 }}>
                                        <thead>
                                            <tr>
                                                {['Campaign Name', 'Influencer', 'Price / Budget', 'Uploaded Media Link', 'Days Ran', 'Analytics'].map((heading) => (
                                                ...
                                                return (
                                                    <tbody key={item.campaignId}>
                                                        <tr style={{ background: expanded ? 'rgba(194,52,10,0.04)' : 'transparent' }}>
                                                            ...
                                                            <td style={{ padding: '14px 12px', borderBottom: `1px solid ${BORDER}` }}>
                                                                <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: TEXT }}>{item.influencer}</p>
